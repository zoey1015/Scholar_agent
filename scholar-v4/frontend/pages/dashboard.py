...
<style>
    ...
    .notif-card { padding: 10px 14px; margin: 6px 0; border-radius: 8px;
                  background: #eff6ff; border: 1px solid #bfdbfe; font-size: 0.88rem; }
    .state-card { padding: 10px 14px; margin: 5px 0; border-radius: 8px;
                  border: 1px solid #e5e7eb; font-size: 0.88rem; }
    ...
    .rel-card { padding: 8px 12px; margin: 4px 0; border-radius: 6px;
...
def load_dashboard():
    try:
        r = httpx.get(f"{API_BASE}/research/dashboard", timeout=10.0)
        if r.status_code == 200:
            return r.json()
    except Exception as e:
        st.error(f"无法加载看板数据: {e}")
...
if notifications:
    ...
    for n in notifications:
        ...
        st.markdown(
            ...
            f'<b>{n.get("title", "")}</b><br>'
            f'<small>{n.get("detail", "")}</small>'
            f'</div>',
            unsafe_allow_html=True,
        )
...
if not items:
    ...
    other_items = [i for i in items if i.get("status") != "open"]
    ...
    if open_items:
        ...
        for item in open_items:
            ...
            with col1:
                st.markdown(
                    f'<div class="state-card state-{status}">'
                    f'{icon} <b>[{itype}]</b> {item.get("content", "")}'
                    f'</div>',
                    unsafe_allow_html=True,
                )
            with col2:
                ...
                if st.button("归档", key=f"archive_{item_id}"):
                    try:
                        httpx.post(
                            f"{API_BASE}/research/state/{item_id}/status",
...
if not relations:
    ...
    type_counts = {}
    for r in relations:
        rt = r.get("relation_type", "other")
        type_counts[rt] = type_counts.get(rt, 0) + 1

    cols = st.columns(len(type_counts) if type_counts else 1)
    for i, (rt, count) in enumerate(type_counts.items()):
    ...
    for r in relations:
        ...
        st.markdown(
            ...
            f'<b>{label}</b>: {r.get("summary", "")}'
            f'<br><small>置信度: {r.get("confidence", 0):.1%}</small>'
            f'</div>',
            unsafe_allow_html=True,
        )
